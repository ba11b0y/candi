...
\begin{abstract}
%-------------------------------------------------------------------------------
Large Language Models (LLMs) have become more capable at complex tasks, leading to widespread integration across coding, education, legal assistance, customer support, etc. With increased adoption, they now access vast amounts of sensitive data. Prior work \cite{cywinski2025towards} shows that white-box mechanistic interpretability can elicit latent knowledge, but most studies rely on model organisms \cite{marks2025auditing} explicitly trained to hold secrets. In real-world scenarios, models are instead aligned and privacy-tuned post-training. This project examines privacy-preserving properties in a realistic setting: a model trained for PII redaction on a PII masking dataset, then probed with mechanistic interpretability (activation and circuit analyses) to elicit PII information after redaction training. The pipeline includes fine-tuning with LoRA, feature extraction via sparse autoencoders (SAEs), and feature ablation. Results show that ablating a single SAE feature (in our case, feature 3867) significantly degrades masking performance and increases PII leakage, demonstrating causal links between interpretable features and privacy behavior. Code: \href{https://github.com/ba11b0y/candi}{https://github.com/ba11b0y/candi}.
\end{abstract}
...
Neural networks often encode more conceptual features than there are physical neurons, a phenomenon that complicates interpretability \cite{monosemantic_features}. Sparse Autoencoders (SAEs) address this by disentangling these superimposed features. We utilized a pretrained SAE based on \texttt{gpt2-small-res-jb}, targeting the residual stream at layer 7 \texttt{(blocks.7.hook\_resid\_pre)}. This SAE expands the original 768-dimensional space into 24,576 features (a 32× expansion). Using the \texttt{HookedSAETransformer} library, we intercepted the layer 7 activations during inference; these were then encoded by the SAE, where a ReLU activation ensured a sparse feature representation.
